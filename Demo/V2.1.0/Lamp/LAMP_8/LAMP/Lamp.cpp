...
VOID CLamp::Initialize(ICOMPONENT* cpt, DSIMMODES smode)
{
	...
	COLORREF color = 0;
	if (component->GetColorFieldById(100, color))      //通过ID值获取用户定义的颜色
	{
		m_fs.color = color;
	}
}
...
{
	char tmp[10] = "1";                                                  //引脚编号
	component->GetSimPinByNumber(input_pin, component->GetTCHARFromChar(tmp));
}
...
BOOL CLamp::Simulate(ABSTIME time, RUNMODES mode)
{
	...
	if (component->PinDataValid())                             //如果已经采集到数据
	{
		BYTE dat;
		component->GetPinData(dat, input_pin);                   //获取该引脚的数据
		pro_data_tmp.data = dat;
		component->AddProData(pro_data_tmp);       //将该引脚数据添加到预置数据列表
	}

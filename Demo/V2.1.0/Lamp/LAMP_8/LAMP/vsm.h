...
/*****                                                          *****/
/*****                                                          *****/
/*****       VisuaulCom System Modeling Interface Header        *****/
/*****                                                          *****/
/********************************************************************/
/********************************************************************/
...
// Version 1.01 - Added Remote Target Control API to ICOMPONENT
...
// Remote Pin Trigger Type Definition
enum RPINTTYPE
{
	RPTT_NONE = 0,
	RPTT_RISING = 1,
...
typedef std::list<SIMPIN*> PinCollection;
typedef std::vector<BYTE> pmodeColection;
struct SIMBUS
{
	PinCollection p;
};
...
struct ICOMPONENT
{
	...
	virtual BOOL GetColorFieldById(INT id, COLORREF& color) = 0;
	...
	virtual VOID GetSymbolOffset(INT index, DPOINT& pt) = 0;
	virtual DPOINT GetLastSymbolOffset() = 0;
	virtual VOID GetLastSymbolOffset(DPOINT& pt) = 0;
	...
	virtual VOID SetSampleTimeInterval(const ULONGLONG& t) = 0;
	virtual VOID GetSampleTimeInterval(ULONGLONG& t) = 0;
	virtual TCHAR* GetAppDirectory(TCHAR* subfolder, BOOL reverse_flag) = 0;
	virtual VOID GetSystemTime(TIME& t, INT dst = -1) = 0;
	virtual LONG GetThreadTimeInterval() = 0;
	virtual BOOL GetProData(PRODATA& pro_data, BOOL backup = TRUE) = 0;
	virtual BOOL AddProData(PRODATA& pro_data, BOOL backup = TRUE) = 0;
	...
	virtual VOID SetOutputRun(TCHAR* str) = 0;
	...
	virtual BOOL GetSimPinByNumber(SIMPIN& p, TCHAR* num) = 0;
	virtual BOOL GetSimPinByName(SIMPIN& p, TCHAR* name) = 0;
	virtual BOOL IsLogicHigh(const SIMPIN& pin) = 0;
	virtual BOOL IsLogicLow(const SIMPIN& pin) = 0;
	virtual BOOL IsPosEdge(const SIMPIN& pin) = 0;
	virtual BOOL IsNegEdge(const SIMPIN& pin) = 0;
	virtual BOOL IsEdge(const SIMPIN& pin) = 0;

	virtual BOOL PinDataValid() = 0;
	virtual VOID EnableOverflowData() = 0;
	virtual BOOL GetPinData(BYTE& dat, SIMPIN& pin) = 0;
	virtual BOOL GetPinDataByType(BYTE& dat, SIMPIN& pin, const SIMPIN& spin, RDATATYPE type) = 0;
	virtual BOOL GetHostReadPinData(BYTE& dat, SIMPIN& pin, const SIMPIN& spin) = 0;
	virtual BOOL GetEventPinData(BYTE& dat, SIMPIN& pin) = 0;
	virtual BOOL GetNormalPinData(BYTE& dat, SIMPIN& pin) = 0;
	virtual BOOL GetOverflowPinData(BYTE& dat, SIMPIN& pin) = 0;
	virtual BOOL GetPinDefaultData(BYTE& dat, SIMPIN& pin) = 0;
	virtual BOOL GetPinDefaultMode(BYTE& mode, SIMPIN& pin) = 0;
	virtual BOOL GetPinAssignMask(BYTE& dat, const SIMPIN& pin, BOOL flag = TRUE) = 0;

	virtual BOOL GetPinBusData(ULONGLONG& dat, SIMBUS& bus) = 0;
	virtual BOOL GetPinBusDataByType(ULONGLONG& dat, SIMBUS& bus, const SIMPIN& spin, RDATATYPE type) = 0;
	virtual BOOL GetHostReadPinBusData(ULONGLONG& dat, SIMBUS& bus, const SIMPIN& spin) = 0;
	virtual BOOL GetEventPinBusData(ULONGLONG& dat, SIMBUS& bus) = 0;
	virtual BOOL GetNormalPinBusData(ULONGLONG& dat, SIMBUS& bus) = 0;
	virtual BOOL GetOverflowPinBusData(ULONGLONG& dat, SIMBUS& bus) = 0;
	virtual BOOL GetPinBusDefaultData(ULONGLONG& dat, SIMBUS& bus) = 0;
	virtual BOOL GetPinBusDefaultMode(pmodeColection& pm, SIMBUS& bus) = 0;
	virtual BOOL GetPinBusAssignMask(ULONGLONG& dat, SIMBUS& bus, BOOL flag = TRUE) = 0;

	virtual VOID ReadPinData(SIMPIN pin) = 0;
	virtual VOID SetPinData(SIMPIN& pin) = 0;
	virtual VOID SetPinData(SIMPIN& pin, BYTE dat) = 0;
	virtual VOID SetPinBusData(SIMBUS& bus) = 0;
	virtual VOID SetPinBusData(SIMBUS& bus, ULONGLONG d) = 0;
	virtual VOID SetPinMode(SIMPIN& pin, RPINMODE mode) = 0;
	virtual VOID SetPinBusMode(SIMBUS& bus, RPINMODE mode) = 0;
	virtual VOID SetPinTriggerType(SIMPIN& pin, RPINTTYPE type) = 0;
	virtual VOID AddPinToBus(SIMBUS& bus, SIMPIN& pin) = 0;
	virtual VOID SetPinDataHigh(SIMPIN& pin) = 0;
	virtual VOID SetPinDataLow(SIMPIN& pin) = 0;
	virtual VOID SetPinDataDelay(WORD delay) = 0;
	virtual VOID SetDataDelay(DWORD delay) = 0;

	virtual VOID EnableClockGenerator(BOOL flag = TRUE) = 0;
	virtual VOID SetClkGenOutputPin(SIMPIN& pin) = 0;
	virtual VOID SetClkGenOutputPin(SIMBUS& bus) = 0;
	virtual VOID SetClkGenIntervalTime(ULONGLONG f, ULONGLONG b = 0) = 0;
	
	virtual VOID EnableCntlr(WORD cntlr, BYTE index, BOOL flag = TRUE) = 0;
	virtual VOID EnableCntlr(PARAMS& pc) = 0;
	virtual VOID SetCntlrParam(WORD cntlr, WORD which, BYTE index, ULONGLONG& value) = 0;
	virtual VOID SetCntlrParam(PARAMS& pc) = 0;
	virtual BOOL GetCntlrValue(WORD cntlr, WORD which, BYTE index, ULONGLONG& value) = 0;
	virtual BOOL GetCntlrValue(PARAMS& pc) = 0;
